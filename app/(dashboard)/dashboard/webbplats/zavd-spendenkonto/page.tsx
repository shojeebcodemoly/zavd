...
const defaultValues: FormData = {
	hero: {
		taglineDe: "ZAVD Verband",
		taglineEn: "ZAVD Association",
		titleDe: "ZAVD Spendenkonto",
		titleEn: "ZAVD Donation Account",
		subtitleDe: "Unterstützen Sie die Bildungs-, Integrations- und Kulturarbeit des ZAVD in Deutschland und Europa.",
		subtitleEn: "Support the educational, integration and cultural work of ZAVD in Germany and Europe.",
		image: "/images/donate/Association1.jpg",
	},
	left: {
		sectionTitleDe: "Spendenkonto - ZAVD Verband",
		sectionTitleEn: "Donation Account - ZAVD Association",
	...
	middle: {
		...
		whyTextDe: "Der Zentralverband der Assyrischen Vereinigungen in Deutschland e.V. (ZAVD) fördert die soziale, kulturelle und gesellschaftliche Integration der Assyrer in Deutschland und Europa. Ihre Spende unterstützt Bildungsprogramme, Flüchtlingshilfe und Integrationsarbeit.",
		whyTextEn: "The Central Association of Assyrian Organizations in Germany e.V. (ZAVD) promotes the social, cultural and civic integration of Assyrians in Germany and Europe. Your donation supports educational programs, refugee assistance and integration work.",
	...
	right: {
		...
		purposeValueDe: "Spende Verband",
		purposeValueEn: "Donation Association",
		iban: "DE52 4785 0065 0000 8003 19",
		bic: "WELADED1GTL",
...
export default function ZavdSpendenkontoAdminPage() {
	...
	useEffect(() => {
		async function fetchData() {
			try {
				const res = await fetch("/api/zavd-spendenkonto-page");
				if (res.ok) {
	...
	const onSubmit = async (data: FormData) => {
		...
		try {
			...
			if (res.ok) {
				toast.success("ZAVD Spendenkonto page saved successfully");
			} else {
	...
	return (
		<div className="space-y-6">
			<div className="flex items-start justify-between">
				<div>
					<h1 className="text-3xl font-medium tracking-tight">ZAVD Spendenkonto</h1>
					<p className="text-muted-foreground">
						Manage the ZAVD Spendenkonto page content in German and English.
					</p>
				</div>
				<a
			...
			<form id="zavd-spendenkonto-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
							...
							<CardContent className="space-y-6">
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
										...
										<Input {...form.register("hero.taglineDe")} value={form.watch("hero.taglineDe") || ""} placeholder="ZAVD Verband" />
									</div>
									<div className="space-y-2">
										<Label>Tagline (EN)</Label>
										<Input {...form.register("hero.taglineEn")} value={form.watch("hero.taglineEn") || ""} placeholder="ZAVD Association" />
									</div>
								</div>
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
										<Label>Title (DE)</Label>
										<Input {...form.register("hero.titleDe")} value={form.watch("hero.titleDe") || ""} placeholder="ZAVD Spendenkonto" />
									</div>
									<div className="space-y-2">
										<Label>Title (EN)</Label>
										<Input {...form.register("hero.titleEn")} value={form.watch("hero.titleEn") || ""} placeholder="ZAVD Donation Account" />
									</div>
								</div>
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
							...
							<CardContent className="space-y-6">
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
										...
										<Input {...form.register("left.sectionTitleDe")} value={form.watch("left.sectionTitleDe") || ""} placeholder="Spendenkonto - ZAVD Verband" />
									</div>
									<div className="space-y-2">
										<Label>Section Title (EN)</Label>
										<Input {...form.register("left.sectionTitleEn")} value={form.watch("left.sectionTitleEn") || ""} placeholder="Donation Account - ZAVD Association" />
									</div>
								</div>
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
							...
							<CardContent className="space-y-6">
								...
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
										...
										<Input {...form.register("right.purposeValueDe")} value={form.watch("right.purposeValueDe") || ""} placeholder="Spende Verband" />
									</div>
									<div className="space-y-2">
										<Label>Purpose Value (EN)</Label>
										<Input {...form.register("right.purposeValueEn")} value={form.watch("right.purposeValueEn") || ""} placeholder="Donation Association" />
									</div>
								</div>
								<div className="grid gap-4 md:grid-cols-2">
									<div className="space-y-2">
										<Label>IBAN</Label>
										<Input {...form.register("right.iban")} value={form.watch("right.iban") || ""} placeholder="DE52 4785 0065 0000 8003 19" />
									</div>
									<div className="space-y-2">
										<Label>BIC</Label>
