...
export default async function NachrichtenDetailPage({ params }: Props) {
			...
			<section className="relative w-full h-64 md:h-80 flex items-end justify-center overflow-hidden">
				...
				<div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 text-white">
					...
					<div className="flex flex-wrap items-center gap-4 text-sm text-white/80">
						...
						{category && (
							<span className="bg-primary text-white text-xs font-semibold px-3 py-1 rounded-sm">
								{category}
							</span>
						)}
					</div>
						...
						<div className="space-y-6">
							...
							<div className="bg-white rounded-xl shadow-sm p-5">
								...
								<div className="space-y-3">
									{recentArticles.filter((a) => a.id !== article.id).slice(0, 4).map((a) => (
										<Link
											key={a.id}
											href={`/aktuelles/nachrichten/${a.slug}`}
											className="block group"
										>
											<p className="text-sm font-medium text-slate-800 group-hover:text-primary transition-colors line-clamp-2 leading-snug">
												{a.title}
											</p>
							...
							{(siteSettings?.phone || siteSettings?.email) && (
								<div
									...
									style={{
										background: siteSettings?.contactBackground
											? `url(${siteSettings.contactBackground}) center/cover no-repeat`
									...
									<div className="bg-black/50 p-5">
										...
										{siteSettings?.phone && (
											<a
												...
												className="flex items-center gap-2 text-white text-sm mb-2 hover:text-primary transition-colors"
											>
										...
										<Link
											href="/kontakt"
											className="mt-4 block text-center bg-primary text-white text-xs font-semibold px-4 py-2 rounded hover:bg-primary/90 transition-colors"
										>
											Contact Us
										</Link>
									</div>
								...
								<div className="bg-white rounded-xl shadow-sm p-5">
									...
									<ul className="space-y-2">
										{categories.map((cat) => (
											<li key={cat}>
												<Link
													href={`/aktuelles/nachrichten?category=${encodeURIComponent(cat)}`}
					...
					<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
							{recentArticles.filter((a) => a.id !== article.id).slice(0, 3).map((a) => {
								const aDate = new Date(a.publishedAt).toLocaleDateString("en-US", {
									year: "numeric",
									month: "short",
									day: "numeric",
								});
								return (
									<div key={a.id} className="group">
										...
										<h4 className="text-sm font-bold text-slate-900 mb-1 line-clamp-2 group-hover:text-primary transition-colors">
											{a.title}
										</h4>
										<p className="text-xs text-slate-400 mb-2">{aDate}</p>
										{a.excerpt && (
										...
										<Link
											href={`/aktuelles/nachrichten/${a.slug}`}
											className="inline-flex items-center gap-1 bg-primary text-white text-xs font-semibold px-3 py-1.5 rounded hover:bg-primary/90 transition-colors"
										>
