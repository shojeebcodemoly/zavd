...
export default async function NachrichtenPage({ params }: Props) {
	...
	const heroTitle = isEn
		? (page.hero.titleEn || "News")
		: (page.hero.titleDe || "Nachrichten");
			...
			<section className="relative bg-[#0f1e2e] py-14 overflow-hidden">
				{press?.backgroundImage ? (
					...
					<div
						...
						style={{
							backgroundImage: `radial-gradient(circle, #4a7fa5 1px, transparent 1px)`,
						...
						<>
									...
									return (
										...
										>
											...
											<div className="relative h-52 w-full overflow-hidden">
												{article.featuredImage?.url ? (
													<>
														...
														/>
														{category && (
															<span className="absolute bottom-2 left-2 bg-primary text-white text-xs font-semibold px-3 py-1 rounded-sm z-10">
																{category}
															</span>
														)}
													</>
											...
											<div className="p-5">
												...
												<Link
													href={`/aktuelles/nachrichten/${article.slug}`}
													className="inline-flex items-center gap-1.5 bg-primary text-white text-xs font-semibold px-4 py-2 rounded hover:bg-primary/90 transition-colors"
												>
							...
							{articles.length > 9 && (
								<div className="text-center mt-12">
									<button className="text-sm text-slate-500 hover:text-primary transition-colors underline underline-offset-4">
										More Blogs
									</button>
								</div>
							)}
