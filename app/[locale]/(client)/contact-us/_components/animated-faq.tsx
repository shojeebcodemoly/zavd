...
export function AnimatedFAQ({ data, isEn = false }: AnimatedFAQProps) {
		...
		<section className="section-padding bg-muted">
			<div className="_container">
				...
				>
					...
					<p className="mb-12 text-lg text-foreground/70">{subtitle}</p>
				</motion.div>

				<div className="mx-auto grid max-w-5xl gap-6 md:grid-cols-2">
					{data.faqs.map((faq, index) => {
						const question = isEn ? (faq.questionEn || faq.questionDe) : (faq.questionDe || faq.questionEn);
						const answer = isEn ? (faq.answerEn || faq.answerDe) : (faq.answerDe || faq.answerEn);
						...
						return (
							...
							>
								<div className="mb-3 flex items-start gap-3">
									...
									<h3 className="font-medium text-secondary">{question}</h3>
								</div>
								<p className="pl-11 text-sm text-foreground/70">{answer}</p>
							</motion.div>
						);
					})}
				</div>
